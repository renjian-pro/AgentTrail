...
> ## 2026-08-06 复核更新
>
> 用户反馈"这份文档感觉有不少内容已经过时了"，逐条核对了本文档引用最多的第 3 节问题清单，
> 结论：**判断框架和目标架构（第 4-10 节）仍然成立**，但第 3 节里描述"当前状态"的几条已经
> 部分或整体过时——这三天里代码在往这份文档指的方向走，只是没有照着这份文档的措辞走。
> 没有逐条核对第 1-2 节的调用链描述，只重点核对了下面这几条影响判断的（尤其是会误导"现在
...
>   并阻塞等待"。这条当初列的严重度很高，现在已经不构成继续加能力时的阻塞项。
> - **P0-7（任务模型不统一）描述要更新，但问题本质没解决**：不再是"Research 没有持久
>   Task"——现在两边都有 `taskId` 语义了，但两套模型深浅不一：`PptTaskStore` 有 `JdbcPptTaskStore`
>   落库实现（`ppt_generation_task` 表，重启和多实例都能看到）；`DeepResearchTaskRegistry` 纯
>   `ConcurrentHashMap`，重启即丢（`DeepResearchController.status()` 的报错文案自己都写着"可能是
>   应用重启丢失了进行中任务的记录"）。这条从"有没有任务模型"变成了"两套任务模型持久化程度不
>   对等"，仍然是一个真问题，只是描述要改。
> - **P0-4（telescoping constructor）部分缓解**：`AgentLoopExecutor` 现在有一个 `Builder`
...
> 至今没有任何超时，和 P0-2 描述的风险是同一件事的另一个具体表现，见踩坑点 #92。
>
> **本文档没有覆盖、但方向一致的新变化**：2026-08-06 新增了 Golden Case 管理/badcase 提升
> （`evaluation.GoldenCase*`）和一个同步模型调用的统一超时出口 `loop.core.SynchronousLlmCall`——
> 后者恰好是本文档 Phase 2 想要的 `ModelGateway` 方向的一小块雏形（先解决超时这个战术问题，
> 没有引入新的抽象层），迁移到 `ModelGateway` 时会被自然吸收，不是要推倒重做的代码。
>
> **结论**：不需要因为部分过时就重写这份文档——第 4 节之后的目标架构、分层方式、10 个阶段的
> 计划和第 12 节的落地顺序判断依然是对的，把它当"设计蓝图"读没问题；只是把第 3 节当"现状快照"
> 直接引用时，上面四条要按这次复核的说法读，不要照抄原文本的问题描述。
