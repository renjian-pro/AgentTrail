# DataAgent 工程约束审计（2026-08-03）

## 结论

本次审计聚焦数据分析领域的工程约束，而不是复用某个 Agent Runtime 的代码结构。结论由 AgentTrail 当前架构、SQL 安全边界、认证需求和可恢复性目标共同推导；Runtime 选型只承担执行职责，不能代替业务安全与状态治理。
...
2. 重构蓝图把 DataAnalysis 泛化为 Workflow；但分析路径是开放式 ReAct，七个阶段是 SOP 而不是必须逐节点执行。Workflow 只包裹长任务、队列、checkpoint 和 Artifact，不替代 Agent 的决策循环。
...
- 不接受任何明文密码比较或存储的开发期简化；应使用 BCrypt/Argon2、统一登录失败提示并限流，避免账号枚举和暴力破解。
...
- 配置、示例代码和导出文档中不得出现凭据样式的 API Key、数据库或 Redis 密码。发现后无论是否仍有效，都应立即撤销/轮换，改用环境变量或密钥管理，并清理历史提交。
...
- AgentTrail 当前的 Runtime、认证、任务、检查点、Artifact 和审计边界。
- SQL AST 解析、最小权限、默认拒绝、结果脱敏、查询资源限制等通用安全原则。
