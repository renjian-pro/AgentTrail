...
> **2026-07-31 更新（第二次修正）**：issue #1-#14 已全部关闭，但这不等于"引擎"已经对齐参考框架
> `docs/core/` 下完整的 19 篇能力文档——核对那份清单后发现，下面表格里原来标成"Phase 3"/
> "Phase 8"/不分阶段的 6 项里，有 5 项其实是 core/05、06、17、18、01 对应的机制，参考框架里
> 通用、不含业务知识，跟本表其余 Phase 0 行是同一量级，**之前的分类是错的**，不是"以后有空再做
> 的业务层增强"。已经改成 `0.x` 归位；只有"结构化输出"里"高危输出走 Reviewer Agent 二次确认"
> 这一半是本项目自己在 core/13 基础机制之上加的业务层判断，保留在 Phase 3。SubAgent（core/18）
> 机制本身也不需要等任何 Capability Pack——Phase 8 的"等 2-3 个能力包"只对"拿真实业务 Agent
> 演示"这件事成立，对"把机制本身写出来"不成立，所以状态列单独说明。
>
> 结论：**Phase 0 + Phase 1 并没有整体做完**，issue #1-#14 做的是"引擎的一部分"，不是全部。
> 下面表格已同步；每行的 issue 号可以在 `gh issue view <n> --json state,body` 里查到验收标准。
...
| **0.11** | **同步（非流式）调用**（对应 core/01；agentx 的 `call`/`callForResult` 和流式是同等地位的入口，不是可选项） | ❌ 未实现——之前误标"不分阶段"，实际是引擎基础能力缺口 |
| **0.12** | **分层记忆体系**（短期会话历史/用户画像&偏好/跨会话语义摘要三层，对应 core/05） | ❌ 未实现——之前误标 Phase 3，参考框架里是通用、不含业务知识的一等公民机制 |
| **0.13** | **分阶段输出 / Timeline**（`StageOutputProvider` SPI + 循环在固定生命周期点调用，对应 core/06） | ❌ 未实现——之前误标 Phase 3，是接在 `AgentLoopExecutor` 本身上的通用埋点机制 |
| **0.14** | **TraceAudit 追踪审计**（每轮 Prompt/工具调用/耗时/Token 落库，对应 core/17） | ❌ 未实现——之前误标 Phase 3，审计对象是"任意一轮"，不含业务知识 |
| **0.15** | **结构化输出（JSON Schema 注入 + 自动修复）**（对应 core/13 的基础机制部分） | ❌ 未实现——之前误标 Phase 3；"高危输出走 Reviewer Agent 二次确认"那一半仍然是 Phase 3（见下方 Phase 3 表），但**这一条基础机制本身要先有**，Reviewer 判断才有东西可判断 |
...
> **状态（2026-07-31 修正）**：issue #1-#14 全部关闭，但对照参考框架 `docs/core/` 完整的 19 篇
> 能力文档核对后发现，Phase 0 状态表里还有 0.11-0.15 五项（同步调用、分层记忆、分阶段输出、
> TraceAudit、结构化输出基础机制）之前被错误分类成"Phase 3/8 的业务层增强"，其实是引擎本身
> 还没做完的部分——不是这条依赖已经清空，是**这条依赖之前被误判为清空了**。下面这条硬约束
> 严格来说要等 0.11-0.15 也做完才真正清空；如果不打算把 AgentTrail 和参考框架的能力集完全对齐，
> 也可以有意识地跳过其中几项直接开 Capability Pack——但那是"知情后选择跳过"，不是"这些本来
> 就不算引擎的一部分"。
...
**issue #1-#14 全部关闭，`mvn test` 全绿（含真实 MySQL/Redis 的 Testcontainers 集成测试），
但这只是"引擎"的一部分，不是全部**——对照参考框架 `docs/core/` 完整 19 篇能力文档核对后，
Phase 0 状态表里的 0.11-0.15 五项（同步调用、分层记忆、分阶段输出、TraceAudit、结构化输出
基础机制）之前被错误分类成"以后有空再做的业务层增强"，实际上和已经做完的 0.1-0.10 是同一
量级的引擎能力，还没有对应的 issue、也还没写。

**接下来两条路，自己选：**

- **A. 先把引擎和参考框架的能力集对齐**：给 0.11-0.15 各开一个 issue（照 #1-#14 的格式），
  做完再开 Capability Pack。好处是"引擎"这个词名副其实，面试被问"这五项呢"时不会露怯；
  代价是要再花一轮才能碰到 SQL/RAG 这些更有产品形态的东西。
- **B. 知情跳过，先做 Capability Pack**：0.11-0.15 里有几项（尤其分层记忆、TraceAudit）在
  实际做 Phase 2/4/5 时大概率会自然长出来（比如 SQL 包想做"记住用户上次问过的表"就是分层
  记忆的一个子集），到时候回头补个 issue、把这次讨论里已经查清楚的参考实现路径接上即可，
  不用现在就单独做一轮。风险是如果一直不回头补，"引擎"这个说法会一直和实际不符。

已知的、故意留到后续的缺口（不管选哪条路都不阻塞，但动到对应机制时要记得补上）：
