...
1. **项目定位变化**：本项目的核心价值是面试作品——"每一层是怎么转的、为什么这么设计"比"覆盖面最广"更有说服力，尤其是 10 年+ 后端架构经验转 AI/Agent 方向时，招聘官更想看到的是"你真的理解 Agent Loop 的内部机制"，而不是"你会接入一个新框架"。框架的自动执行会把"我是否真的理解这一层"这件事藏起来。
...
- **维持 ADR 0001（继续用 AgentScope Java 2.0 自动执行）**：否决。接入一个新框架、摸清它的自动执行边界情况（重试策略、错误分类、流式事件粒度）本身是一笔不小的学习成本，且"框架自动执行"这条叙事在面试里天然弱于"我理解并实现了这套机制"。
...
- 面试叙事从"框架优先，只有验证过框架做不到才自己写"变成"先手写证明理解，再有计划地在合适的节点引入框架"——这本身是一次值得如实讲述的架构决策演进，包括"为什么会先选框架、又为什么推翻"。
- 现有 `com.agenttrail.loop.AgentLoop`（含配套测试 `AgentLoopTest`/`ScriptedLlmClient`/`RecordingTool`）保留为"V0 简化版对比参考"，不删除；`AgentScopeRuntime`/`agentscope-bom` 依赖同样保留为 V2 候补（面试时可用来对比"从最小实现到生产级实现多了什么"）。
