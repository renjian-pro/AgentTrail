...
代码注释、commit message 和文档只记录 AgentTrail 的领域约束、架构决策与验证证据，不包含私人素材、本机路径或历史项目名。见 `AGENTS.md`。
...
工程约束审计（[DataAgent 工程约束审计](../../dataagent-design-audit-2026-08-03.md)）确认，以下三项在当前项目中没有完整方案，需要由 Phase 2B 新增设计：

| 主题 | 落在哪张票 |
|---|---|
| DataAgent 领域评测体系（Golden SQL、越权与脱敏回归） | 13 |
...
顺带一个架构收益：Phase 2A 的部门范围解析走 `ancestors LIKE '前缀,%'` 实时查询，**不存在**“进程内邻接树在多节点下如何保持一致”的问题——这是选型带来的连锁简化，不需要额外设计。
