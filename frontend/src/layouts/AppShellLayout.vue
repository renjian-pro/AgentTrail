...
<template>
  <main :class="{ dark }" class="shell">
    <aside class="sidebar">
      ...
      <div class="history"><small>Recent conversations</small><button v-for="session in chat.sessions" :key="session.id" :class="{ selected: session.id === chat.conversationId }" @click="openChat(session.id)"><span class="kind-mark">{{ chat.agentKindFor(session.id) === 'analytics' ? '⌁' : '○' }}</span>{{ session.title }}</button><span v-if="!chat.sessions.length" class="empty-history">No conversations yet</span><button v-if="chat.sessionsHasMore" class="load-more" @click="chat.loadMoreSessions">Load more</button></div>
