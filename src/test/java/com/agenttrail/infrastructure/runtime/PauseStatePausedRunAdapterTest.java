...
class PauseStatePausedRunAdapterTest {
    ...
    void projectsOwnershipVariantAndSanitizedPendingToolsFromTheSnapshot() {
        ...
        store.save(new PauseState("conv-1", List.of(),
                ...
                PauseReason.HITL_APPROVAL, ResumeSafePoint.BEFORE_TOOL_EXECUTION, "write",
        ...
        assertThat(paused.safePoint()).isEqualTo(ResumeSafePoint.BEFORE_TOOL_EXECUTION);
    ...
    @Test
    void exposesAfterToolExecutionSoTheClientCannotOfferAFalseRejection() {
        InMemoryPauseStateStore store = new InMemoryPauseStateStore();
        store.save(new PauseState("conv-2", List.of(), List.of(),
                PauseReason.HITL_APPROVAL, ResumeSafePoint.AFTER_TOOL_EXECUTION, "write",
                new RunnableParams("conv-2", "user-1"), "deepseek-chat", 2, 99L));

        var paused = new PauseStatePausedRunAdapter(store, ToolRiskRegistry.defaults())
                .find("conv-2").orElseThrow();

        assertThat(paused.safePoint()).isEqualTo(ResumeSafePoint.AFTER_TOOL_EXECUTION);
        assertThat(paused.pendingTools()).isEmpty();
    }
}
