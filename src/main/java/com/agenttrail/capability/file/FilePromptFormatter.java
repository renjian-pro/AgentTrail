...
public final class FilePromptFormatter {
    ...
    /** Same prompt contract for the File QA port model; keeps controllers free of FileStore. */
    public static String formatAttachmentSection(List<Attachment> files) {
        if (files == null || files.isEmpty()) {
            return "# 会话文件\n\n当前会话没有已上传的文件，不要调用 load_file_content。\n";
        }
        StringBuilder section = new StringBuilder("# 会话文件\n\n");
        section.append("以下文件的内容必须通过 load_file_content 工具获取，禁止在没有调用工具的情况下凭空回答和文件相关的问题。\n");
        appendAttachmentGroup(section, "本轮上传的文件", files.stream().filter(file -> file.turnId() == null).toList());
        appendAttachmentGroup(section, "此前上传的文件", files.stream().filter(file -> file.turnId() != null).toList());
        return section.toString();
    }

    private static void appendAttachmentGroup(StringBuilder section, String heading, List<Attachment> files) {
        if (files.isEmpty()) return;
        section.append("\n## ").append(heading).append('\n');
        for (Attachment file : files) {
            section.append("- ").append(file.fileName()).append("（fileId=").append(file.id())
                    .append(file.kind() == FileKind.IMAGE ? "，图片）\n" : "，文本）\n");
        }
    }
